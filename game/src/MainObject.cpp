...
MainObject::MainObject(GameObject& associated, std::string objectName, int hp, Vec2 scale, bool miscObject, bool destructible) : Component(associated) {
	...
	std::string path;
	if(miscObject)
		path = ASSETS_PATH("/img/objects/" + objectName + "_bottom.png");
	else
		path = ASSETS_PATH("/img/objects/" + objectName + "_full.png");
...
void MainObject::Damage(int damage) {
	...
	if(hp < 1) {
		...
		Sprite* sp;
		if(objectName == "fountain")
			sp = new Sprite(*go, ASSETS_PATH("/img/objects/" + objectName + "_end.png"), 6, 0.1);
		else
			sp = new Sprite(*go, ASSETS_PATH("/img/objects/" + objectName + "_end.png") );
		sp->SetScale(scale);
		go->AddComponent(sp);
