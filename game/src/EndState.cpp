...
EndState::EndState() : State() {
	...
	if(true) {
		...
		if(GameData::playerVictory) {
			sp = new Sprite(*go, ASSETS_PATH("/img/title/message_win.png"), 2, 0.4);
			sp->SetScale(Vec2(3, 3));
			go->AddComponent(sp);
			go->AddComponent(new CameraFollower(*go, Vec2(275, 300) - (go->box.GetSize() / 2)));
		}
		else {
			sp = new Sprite(*go, ASSETS_PATH("/img/title/message_loss.png"), 2, 0.4);
			sp->SetScale(Vec2(3, 3));
			go->AddComponent(sp);
		...
		if(true) {
			...
			go = new GameObject();
			go->AddComponent(new Text(*go, ASSETS_PATH("/font/Sabo-Filled.ttf"), 30, text, SDL_Color {}, Text::SOLID));
			...
			go = new GameObject();
			go->AddComponent(new Text(*go, ASSETS_PATH("/font/Sabo-Filled.ttf"), 30, "saved", SDL_Color {}, Text::SOLID));
		...
		if(true) {
			...
			go = new GameObject();
			go->AddComponent(new Text(*go, ASSETS_PATH("/font/Sabo-Filled.ttf"), 30, "captured", SDL_Color {}, Text::SOLID));
