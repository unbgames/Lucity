...
Attack::Attack(GameObject& associated, std::string owner, std::string name, Vec2 origin, float offset, float lifeTime, float direction, float speed,
	...
	sp = new Sprite(associated, ASSETS_PATH("/img/effects/" + name + ".png"), frameCount, 0.1);
...
void Attack::OnDeath() {
	...
	if(name == "bind") {
		Sprite* sp = new Sprite(*go, ASSETS_PATH("/img/effects/" + name + "_end.png"), 3, 0.1, false, 0.3);
		...
		go->AddComponent(new Sprite(*go, ASSETS_PATH("/img/effects/" + name + "_end.png"), 3, 0.1, false, 0.3));
		go->box.SetCenter(associated.box.GetCenter());
		Game::GetInstance().GetCurrentState().AddObject(go, "EFFECT");
	}
